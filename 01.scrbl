...
如果你使用DrRacket之前已经使用了其它以@hash-lang[]开始，那么DrRacket会记得你上次使用的语言，而不是从@hash-lang[]推断的语言。在这种情况下，使用@menuitem["语言|选择语言……"]（Language|Choose Language…）菜单项去改变。在出现的对话框中，选择第一项，它告诉DrRacket使用通过@hash-lang[]申明在源程序中的语言。仍然要把把@hash-lang[]放在文本区域的顶部的。
